...
收藏写入和删除均保持幂等。城市和景点详情页使用登录用户的最近浏览记录；用户可以一键清空，单条历史不提供永久保存承诺。邮箱验证、重发验证、找回密码、重置密码和新邮箱确认已经实现；开发环境返回本地测试邮件链接，生产环境通过统一 SMTP 适配器投递。
...
`search_attractions` 只读取目标城市的景点资料，并按兴趣标签匹配、票价和排除地点产生候选列表；`select_stops` 根据天数、人数和门票预算选择景点。初始候选超过已填写门票预算时，系统优先保留预算内的相关景点，并明确记录该修复结果；`validate_plan` 返回单日负荷、门票预算、开放时间与交通资料状态。地图、餐饮和交通费用尚未接入时必须标记为未覆盖，不能伪造结果。
...
当前算法版本为 `tool-agent-v1`。运行记录使用应用启动建表兼容模式创建；生产部署前仍须提供对应 Alembic 迁移。
...
已确认的规划任务采用单 Agent 加受控工具架构。执行顺序为：需求读取、景点查询、景点详情读取、预算估算、区域排序、时间与城市校验、草稿保存。每一步的输入、输出、状态和失败信息均记录到 `agent_runs` 与 `agent_tool_calls`，页面可通过既有行程执行记录接口读取。

预算超限时先记录初始候选的预算校验结果，再执行不超过两次的修复；当前实现的修复策略为保留预算内且兴趣相关的低价景点。区域排序只是无地图路由服务时的透明降级策略，交通时间和费用仍标为未覆盖。
...
开发环境可使用 SQLite 加应用层余弦相似度完成小规模演示；生产环境使用 PostgreSQL 与 `pgvector` 保存向量，并建立 `ivfflat` 或 `hnsw` 索引。向量模型版本必须记录在文档或切片元数据中，重换模型时重新生成向量，不能混用。
...
1. 建立文档、切片、检索运行和命中记录的数据迁移与管理员审核页。
2. 先使用 20 至 30 篇审核资料完成 SQLite 小规模检索和来源卡片。
3. 将 `search_guide_knowledge` 接入普通聊天，不接入行程事实工具。
