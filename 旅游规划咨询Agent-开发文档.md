...
| 最近修订 | 2026-09-04：补充分档模块化演进、消息协作和服务拆分路线 |
...
| 用户功能 | 用户主页维护资料、偏好和排除地点，收藏城市/景点/行程，查看最近浏览，管理登录设备，创建、查看历史记录并撤销只读行程分享链接，以及提交评分评论 |
...
系统按“先划分职责，再建立异步协作，最后按需独立部署”的路线演进。当前采用模块化单体，浏览器仍只调用同一个 `/api/v1` HTTP API；不得为了目录拆分而过早建立多个网络服务或多个数据库。

| 档位 | 目标 | 主要工作 | 部署变化 | 完成判定 |
| :--- | :--- | :--- | :--- | :--- |
| A：基础整理 | 职责清晰且有可验证基线 | 修复类型检查；按业务域迁移 Router、Service、Repository；保留 API 路径和同一数据库 | 无 | 前后端检查通过；已迁移模块不再在 `main.py` 承载路由实现 |
| B：任务可靠性 | 耗时任务不依赖 Web 进程 | 生产禁用内联 Worker；增加任务租约、超时回收、重试和失败记录 | 独立规划 Worker | Web 重启不丢失已持久化任务，任务不会被并发重复领取 |
| C：消息协作 | 异步部门通过可靠事件交接 | 引入事务 Outbox、消息中间件、消费者、重试和死信队列 | 增加消息服务和 Worker | 业务提交与事件投递可追踪，重复消息不产生重复结果 |
| D：按需服务拆分 | 为高耗时功能独立扩容 | 优先拆规划、通知、媒体或知识处理 Worker | 仅拆已稳定的高负载模块 | 服务间通过版本化 API 或事件协作，模块有清晰数据所有权 |
| E：持续治理 | 长期可维护和可运营 | 链路追踪、事件版本、监控告警、容量与恢复演练 | 视运行规模调整 | 可以定位一次请求跨 API、Worker 与外部服务的完整执行链 |

A 档分为两个小步。A1 建立模块目录并优先迁移无业务耦合的系统路由，同时消除已知静态类型错误；A2 按 `auth`、`catalog`、`planning`、`itineraries`、`community`、`admin` 的顺序迁移业务路由。每次迁移只移动一个可测试边界，旧 API 路径、响应字段和数据库表保持兼容。

同步 HTTP API 用于登录、查询和必须立即返回的写入；消息事件只用于规划生成、邮件投递、媒体导入、知识索引和榜单重算等允许异步完成的工作。事件必须包含唯一事件 ID、类型、版本、发生时间、链路 ID 和最小化业务载荷；不得传递密码、会话令牌或完整个人资料。
...
| :--- | :--- | :--- |
| 认证落地页 | `/` | 登录、注册和游客浏览入口 |
| 发现首页 | `/discover` | 季节推荐、热门城市、热门景点和入口 |
...
| 我的行程 | `/itineraries` | 保存的行程、修订版本和历史记录 |
| 用户主页 | `/me` | 维护偏好和排除地点，查看并管理收藏与最近浏览 |
| 我的分享 | `/me/shares` | 查看历史分享记录并撤销仍有效的链接 |
...
| 验证邮箱 | `/auth/verify-email`、`/auth/resend-verification` | 使用邮件中的 6 位验证码激活邮箱或重新发送邮件 |
| 找回密码 | `/auth/forgot-password`、`/auth/reset-password` | 申请并完成密码重置 |
| 确认新邮箱 | `/auth/confirm-email-change` | 使用邮件链接确认新的邮箱地址 |

`/` 是认证落地页；全局导航中的登录入口也使用同一认证界面。项目不注册独立的 `/login` 或 `/register` 前端路由，游客浏览使用 `/discover`。
...
MVP 不实现匿名 Agent 会话迁移。游客可以浏览发现、城市、景点、排行和有效的公开分享；访问需要登录的接口返回 `401` 后，前端打开全局登录界面。项目不使用独立的 `/login`、`/register` 页面，也不使用前端 `redirect` 查询参数。
...
- 验证邮件 60 秒内不能重复发送，每个邮箱和 IP 每小时限制次数。6 位验证码与目标邮箱组合后只保存哈希，3 分钟过期且只能使用一次。
...
| 邮件投递记录 | `/admin/email-outbox` | 查看认证邮件的脱敏投递状态、尝试次数和错误码 |
...
- 用户管理仅显示已完成邮箱验证的正式账号，并展示脱敏邮箱、注册时间和最近登录时间。未验证注册记录仅用于验证码校验和邮箱更正，不出现在用户管理列表、总数或搜索结果中。管理员不能查看密码、Token 或默认读取完整私人对话。
...
邮箱验证码、重置密码和修改邮箱使用统一的 `auth_action_tokens`：
...
邮箱验证码只出现在验证邮件中，服务端将目标邮箱和验证码组合后保存哈希；密码重置与邮箱修改的明文操作令牌只出现在邮件链接中，数据库同样只保存哈希。创建新的同用途凭证时使旧凭证失效。认证安全事件另外记录登录成功/失败、退出、全部设备退出、密码重置、邮箱修改和 Token 重用，只保存必要的时间、结果、设备摘要和 IP 网段，不保存密码、Cookie、完整令牌或验证码。

### 6.0.1 认证邮件投递记录 `email_outbox`

每次发送邮箱验证、密码重置或新邮箱确认邮件时，先创建一条投递记录，再调用邮件适配器。记录只保存 `user_id`、用途、收件人 HMAC 指纹、脱敏收件人、邮件主题、状态、尝试次数、重试次数、最后错误码、发送时间和审计时间；不保存验证码、重置令牌、邮件正文、HTML 正文或完整收件人地址。

状态限定为 `pending`、`sent`、`failed`、`simulated`。本地 `console` 邮件模式使用 `simulated`，真实 SMTP 成功使用 `sent`，不可恢复的适配器错误使用 `failed` 并只记录稳定错误码。SMTP 适配器在一次用户请求内最多尝试 3 次，`attempt_count` 和 `retry_count` 反映该次尝试；本期不保存可延迟重投的敏感邮件内容。失败后由用户重新发送，服务端创建新的验证码或令牌、使同用途旧凭证失效并写入新的投递记录。

`GET /api/v1/admin/email-outbox` 仅管理员可访问，分页响应只返回脱敏收件人、状态、次数、时间和错误码，不返回收件人指纹或任何认证凭证。
...
反馈对 `(itinerary_id, user_id)` 唯一，允许用户更新自己的反馈。反馈处理状态限定为 `open -> in_progress -> resolved`，管理员可分派给启用的管理员并向用户回复；用户更新评分或评论时，旧回复失效并回到 `open`。反馈处理的状态、分派和回复变更必须写入审计日志摘要，不保存回复正文。导入批次只能按 `validating -> ready -> importing -> imported | failed` 转换，管理员可以在 `ready` 前取消；同一文件哈希、数据类型和来源的成功批次不得重复导入。排行任务对 `(period, ranking_type, idempotency_key)` 唯一。审计日志只保存字段名和必要摘要，不保存密码、Token、完整导入文件或完整对话内容；普通管理员无权删除审计日志。
...
| `weather.get` | 查询计划日期的目的地天气；预报不可用时标记未知，不编造天气 |
| `geocode.get` | 将景点文字地址转换为经纬度 |
| `route.driving` | 使用已获取的经纬度生成景点间驾车导航路段 |
| `food.nearby` | 查询景点附近 1.5 公里内的餐饮 POI |
| `route.matrix` | 查询每日行程使用的景点间交通时间和距离 |
...
- 规划时固定按以下顺序执行并记录结果：查询计划日期天气，筛选景点，将每个入选景点的文字地址转换为经纬度，使用这些经纬度规划驾车导航，最后查询各景点附近美食。不得向驾车路线接口传递未经地理编码或缓存验证的坐标。
...
- 相邻景点之间必须有路线时间或距离数据。
- 默认推荐 3 个景点；用户可明确指定 1～12 个。行程详情必须展示天气、驾车导航状态和附近美食查询结果，服务不可用时展示未知或不可用原因。
...
注册接口只创建未验证账号，不发送邮件；前端随后进入验证页，由用户单独调用发送验证码接口。注册、发送或重新发送验证码和找回密码接口成功时统一返回 `202 Accepted`，响应文案不说明邮箱是否存在。邮箱验证提交目标邮箱和 6 位验证码，验证码 3 分钟失效、60 秒内不能重发、成功后立即作废；`retry_after_seconds` 必须限制在 `0` 到 `60` 之间，前端倒计时同样不得超过 60 秒。密码重置和新邮箱确认链接中的一次性令牌由前端读取后立即使用 `history.replaceState` 清除地址栏参数，再通过 POST 请求提交；相关页面设置 `Referrer-Policy: no-referrer`，避免令牌随 Referer 泄露。
...
GET /api/v1/attractions/search?q={keyword}
...
首页公开搜索同时查询城市和景点。城市检索匹配城市名称与 `slug`；景点检索匹配景点名称、简介、区域和所属城市名称，最多返回 10 条已启用且所属城市已启用的景点。前端选择城市后进入 `/cities/{slug}`，选择景点后进入 `/attractions/{id}`；回车时优先进入精确匹配项。
...
POST   /api/v1/itineraries/{itinerary_id}/replan/preview
...
用户端当前的 `PUT` 编辑接口使用 `expected_version` 做并发版本检查；编辑前自动保存快照，恢复历史版本会再创建一条恢复前快照。自然语言调整先调用 `POST /replan/preview`，可选 LLM 只能提出受 Schema 限制的操作；未配置模型或需求不完整时返回澄清问题且不会修改行程。用户确认预览后才调用 `POST /replan` 写入。
...
当前实现使用 `itineraries.deleted_at` 标记回收站状态，保留行程日程、修订、反馈和 Agent 关联。用户删除时同步撤销有效分享，并取消已关联且仍为 `queued` 或 `running` 的规划任务；普通行程列表、详情、收藏和公开分享不会返回已删除行程。管理员通过 `/api/v1/admin/itineraries` 分页查看正常或已删除行程，通过 `/restore` 恢复；只有已删除且分享、反馈、修订、Agent 运行、规划任务和收藏关联数均为 0 的记录，才允许管理员彻底删除。恢复不会重新启用删除时已撤销的分享链接。

“加入行程”有两条明确路径：没有现有行程时跳转规划页并把景点写入 `must_visit`；从某个现有行程操作时使用上述 `add` 操作重新规划。不得直接向时间线插入未经开放时间和路线校验的 stop。

创建分享成功返回 `share_id`、只出现一次的完整 `share_url` 和 `expires_at`。`GET /api/v1/shares` 仅返回当前用户的分享历史元数据：关联行程、创建时间、有效期和有效/失效/撤销状态；不返回原始令牌、令牌尾部或哈希。默认有效期 30 天，用户可以选择更短期限或手动撤销。为草稿或历史版本创建分享返回 `409 CONFLICT`。

当前实现已强制只有本人名下的 `saved` 行程可以创建分享。公开读取会同时检查令牌未撤销、未过期、行程仍为 `saved` 且所属账号有效；账号注销会在同一事务中撤销该账号全部行程分享。当前分享仍指向行程记录本身，固定版本快照和用户侧历史分享列表仍待实现。
...
收藏写入和删除均保持幂等。城市和景点详情页使用登录用户的最近浏览记录；用户可以一键清空，单条历史不提供永久保存承诺。邮箱验证码、重发验证、找回密码、重置密码和新邮箱确认已经实现；邮箱验证码仅通过邮件投递，不在接口响应或页面中返回。开发环境可使用模拟邮件适配器，生产环境通过统一 SMTP 适配器投递。
...
GET    /api/v1/admin/feedback/assignees
...
邮箱验证码、重置密码、修改邮箱和安全提醒通过统一邮件适配器发送。生产环境使用经过域名验证的 SMTP 或事务邮件服务，配置 SPF、DKIM 和 DMARC；开发环境使用本地邮件捕获服务，不向真实用户发送。

- 发送超时不超过 10 秒，最多自动重试 2 次；接口仍返回防枚举的通用响应，服务器记录不含邮箱全文和令牌的失败类型。
- 邮件模板只使用服务端固定模板和受控变量；重置与邮箱修改链接必须以 `APP_BASE_URL` 开头，禁止把用户输入拼接为任意 URL。
- 原始验证码或重置令牌只存在于当前发送调用和邮件中，不写日志、不进入分析事件；发送失败后用户可以重新申请，新申请会使旧凭证失效。
...
所有表、索引、外键和数据回填必须通过 `backend/migrations/versions/` 中的 Alembic 脚本变更。部署顺序为：备份数据库，执行 `alembic upgrade head`，执行 `alembic check`，再重启 Web 服务和规划 Worker。应用启动时只校验当前数据库修订版本，不执行 `create_all` 或手写 `ALTER TABLE`。

首次接入 Alembic 前已由旧应用自动建表的数据库，需在备份后执行 `alembic stamp head` 登记其基线版本，再运行 `alembic check` 确认结构一致。 `stamp` 不创建、删除或改写业务表；若检查发现差异，必须新增迁移脚本，不能在应用启动时临时修表。常规回滚使用 `alembic downgrade -1`，并在恢复演练环境中验证数据兼容性后才允许用于生产。
...
- [x] 缺失、错误或已轮换的 CSRF Token 无法执行写操作；同源正常请求可以完成登录、刷新和退出。
...
| 版本 | 日期 | 修订内容 |
| :--- | :--- | :--- |
| V1.4 | 2026-09-04 | 增加 A-E 分档实施路线，明确模块化单体、可靠任务、消息协作和按需服务拆分的边界 |
...
用户确认旅行需求后，系统创建 `PlanningJob` 和一条 `AgentRun`，再按以下固定白名单顺序执行：

```text
search_attractions -> select_stops -> validate_plan -> itinerary delivery
```

`search_attractions` 只读取目标城市的景点资料，并按兴趣标签匹配、票价和排除地点产生候选列表；`select_stops` 根据天数、人数和门票预算选择景点。初始候选超过已填写门票预算时，系统优先保留预算内的相关景点，并明确记录该修复结果。随后系统补齐缺失的景点坐标，调用高德 Web 服务查询每段景点间路线和往返跨城路线，并将路线耗时、票价或出租车预估计入预算；公共交通按人数计价，打车和自驾按整车计价，自驾额外按固定油费单价估算。`validate_plan` 按出行日期检查常见营业时间格式、单日负荷、路线结果与完整预算。缺失出发城市、坐标、服务失败或不能解析的营业时间必须标记为部分校验，不能伪造结果。

### 22.2 数据与接口

- `agent_runs` 保存规划任务、结构化需求、算法版本、执行状态和摘要。
- `agent_tool_calls` 按顺序保存每次白名单工具的输入、输出和状态。
- `GET /api/v1/itineraries/{itinerary_id}/agent-run` 仅允许行程所有者读取最近一次执行记录，返回工具步骤和摘要，供行程详情页展示规划依据。

当前算法版本为 `amap-route-v2`。景点路线缓存 24 小时，以“起点景点、终点景点、交通方式、公共交通出行日期”为键；跨城路线以“起点城市、终点城市、交通方式、出行日期”为键单独缓存。公共交通指定日期未返回方案时，会重试高德通用公共交通路线并在结果中标注。运行记录使用应用启动建表兼容模式创建，生产部署前仍须提供对应 Alembic 迁移。

### 22.3 边界

大模型不得直接新增景点、修改预算或写入行程。它只能理解对话、生成交付说明，后续若实现结构化重规划，必须先将模型输出转换为受 schema 限制的操作，再由后端工具校验并执行。

---

## 23. 图片候选与审核流程

图片自动获取采用 Wikimedia Commons 的 MediaWiki API。管理员对未启用的 `MediaAsset` 执行自动查询时，后端以景点名或城市名组成检索词，读取图片 URL、原始页面、作者和许可证元数据。查询服务不需要将第三方密钥写入项目环境变量。

自动查询只会写入 `needs_review` 状态且保持 `is_active=false`。管理员必须人工确认图片内容与目标城市或景点一致、来源和许可证可用，再将记录更新为 `approved` 并启用；此时才同步 `City.image_url` 或 `Attraction.image_url` 到前台。

后台接口：

- `GET /api/v1/admin/media-assets`：管理员查询全部图片记录。
- `POST /api/v1/admin/media-assets/{asset_id}/autofill`：为未启用记录查询候选图。
- `PATCH /api/v1/admin/media-assets/{asset_id}`：编辑元数据、核验状态和展示状态。

本地文件仍存放在 `backend/media/` 下，编辑本地记录时只能使用该目录内的相对路径。已启用图片不支持自动覆盖，替换需要通过后台编辑并重新核验。

## 24. Tool-Agent v2

已确认的规划任务采用单 Agent 加受控工具架构。执行顺序为：需求读取、景点查询、景点详情读取、补齐坐标、高德分段路线、完整预算估算、日期与城市校验、草稿保存。每一步的输入、输出、状态和失败信息均记录到 `agent_runs` 与 `agent_tool_calls`，页面可通过既有行程执行记录接口读取。

预算超限时先记录初始候选的门票预算校验结果，再执行不超过两次的修复；当前实现的修复策略为保留预算内且兴趣相关的低价景点。预算由门票、景点间市内交通、餐饮和住宿组成；餐饮按每人每日固定额度，住宿按两人一间、每晚固定额度计算。往返目的地交通和购物不计入。公共交通无方案时会降级为高德步行路线并明确标示；地图服务失败时保留“待确认”状态。

SSE 业务失败使用 `agent_error` 事件，避免与浏览器 `EventSource.onerror` 冲突。浏览器连接异常时，客户端查询 `PlanningJob` 的持久化终态后再显示失败或恢复提示。

自然语言调整接口通过预览返回受 Pydantic Schema 限制的 `actions`：`set_budget`、`set_days`、`set_preferences`、`remove_attraction`、`replace_attraction`。服务端逐项验证景点城市归属和门票预算后，用户确认才创建行程版本；模型输出无效或需求含糊时返回具体澄清问题，不执行写入。

模型失败诊断只在 `GET /api/v1/admin/agent-status` 返回给管理员；普通 `/api/v1/agent/status` 不返回失败细节。诊断包含安全的失败类别、最近失败时间和 Agent 运行成功/失败计数，不包含密钥、请求地址或原始响应。

### 24.1 管理后台接口兼容

`GET /api/v1/admin/overview` 是管理后台首页的必需资料；`GET /api/v1/admin/agent-status` 只是附加诊断资料。前端必须先显示已成功读取的平台概览；诊断接口在旧后端尚未更新、网络暂时失败或不可用时只隐藏运行状态面板，不能将管理员误导为没有权限。

## 25. 用户管理分页

`GET /api/v1/admin/users` 使用服务端分页，参数为 `page`、`page_size`、`status` 和 `search`。默认每页 20 条，单页最多 100 条；响应返回当前页 `items`、匹配总数 `total`、`page` 和 `page_size`。状态筛选和用户名、邮箱、用户公开 ID 搜索均在数据库查询阶段执行，前端不能再一次性下载全部用户后自行筛选。

## 26. 会话与反馈分页

`GET /api/v1/admin/sessions` 和 `GET /api/v1/admin/feedback` 均固定每页 10 条，返回 `items`、`total`、`page` 和 `page_size`。会话接口保留状态筛选，并支持按会话标题、用户名和邮箱搜索；反馈按创建时间倒序展示。前端在切换会话状态或输入搜索词后回到第 1 页，恢复会话后重新加载当前页。

### 26.1 本地历史测试数据清理

`backend/scripts/cleanup_test_data.py` 只处理用户名完整匹配 `test` 加至少 3 位数字（例如 `test001`）的本地 SQLite 账号及其关联数据。测试代码生成的普通账号统一遵循该格式；专门验证纯数字用户名的用例除外。脚本默认只打印范围；只有显式传入 `--apply` 才会执行删除，并在删除前启用数据库外键检查。相似前缀或包含字母的正常用户名不会被脚本选中。

## 27. 后台内容管理与审计

管理员可通过 `/api/v1/admin/cities`、`/api/v1/admin/attractions` 和 `/api/v1/admin/rankings` 对城市、景点和手工排行进行新增、查询、修改和删除。城市和景点支持 `/import` 批量 JSON 导入：接口先校验整批数据，发现重复标识或不存在的城市时整批拒绝，不会只导入一部分。

城市和景点使用独立的 `is_active` 上架状态。后台先通过状态按钮停用内容，停用内容不会出现在公共城市、景点、媒体、收藏、最近浏览和 Agent 新规划候选中；重新启用后可恢复展示。彻底删除只允许对已停用且没有景点、行程、图片、排行、收藏或最近浏览关联的数据执行，历史关联存在时只能保持停用。旧开发数据库在启动时自动补充状态字段，生产环境仍需正式 Alembic 迁移。

排行写入同时校验对象和展示名次：同一对象只能维护一条排行记录，同一排行类型中正在展示的名次只能被一条记录占用。新增、修改和批量导入都执行相同校验；停用记录可保留原名次，但重新启用时必须解决冲突。手工排行记录启用后，公共 `/api/v1/rankings` 优先返回人工维护结果；没有手工记录时才使用原有启发式排行。

城市、景点和排行的新增、修改、删除、批量导入，以及图片自动查找与编辑、用户启停、会话恢复，都会新增一条 `admin_audit_logs` 记录，保存管理员账号、操作类型、对象、摘要和时间。`GET /api/v1/admin/audit-logs` 仅供管理员分页查看，页面不提供删除入口。

反馈收件箱支持按状态筛选、分派给启用的管理员、填写回复并切换 `open`、`in_progress`、`resolved` 状态。管理员回复会在用户自己的行程评分区域展示；处理操作写入审计日志，但审计摘要不保存回复正文。当前不发送站内或邮件通知，也不提供 SLA、标签、批量分派或管理员间备注。

## 25. RAG 攻略知识库设计（首期资料管理与本地检索已实现）

### 25.1 目标与边界

RAG（检索增强生成）用于让模型在回答前检索经过审核的攻略资料。它解决“城市有什么氛围、适合吃什么、有哪些文化背景、出行要注意什么”这类非结构化知识问题，并为回答提供可展示的来源依据。

RAG 不替代现有受控工具。以下事实必须继续由数据库和校验器决定：

| 数据类型 | 唯一可信来源 | 禁止由 RAG 决定的原因 |
| --- | --- | --- |
| 景点城市归属、标签、门票、游览时长 | `cities`、`attractions` | 需要稳定且可校验的结构化数据 |
| 行程顺序、预算、时间冲突 | Agent 工具和校验器 | 需要保证约束满足与可执行性 |
| 城市文化、美食类型、漫游建议、旅行注意事项 | 审核后的知识库文档 | 内容以文字为主，适合按问题检索 |

因此，运行时遵循：`结构化事实 -> 工具查询`，`攻略与解释 -> RAG 检索`，`最终表达 -> LLM`。模型不得把 RAG 文本中的描述当作可以直接写入行程的事实。

### 25.2 知识来源与审核

首期只收录 20 至 30 篇与北京、上海、成都相关的短攻略，每篇必须有城市、主题、来源名称、来源链接、许可或使用说明、审核状态和更新时间。优先使用旅游局公开资料、博物馆或景区官方资料、经过人工整理的项目自有攻略；不直接抓取未经授权的平台长文。

管理员上传或录入后，文档状态为 `needs_review`。只有管理员确认城市归属、来源和内容后才可设为 `approved` 并参与检索；`rejected`、`archived` 和未审核文档不得返回给用户。

### 25.3 数据模型

建议新增以下表，并通过 Alembic 迁移创建：

| 表 | 核心字段 | 用途 |
| --- | --- | --- |
| `knowledge_documents` | `id`、`city_id`、`title`、`source_name`、`source_url`、`content`、`status`、`updated_at` | 保存原始攻略与来源元数据 |
| `knowledge_chunks` | `id`、`document_id`、`chunk_index`、`content`、`embedding`、`token_count` | 保存按段切分后的可检索内容 |
| `retrieval_runs` | `id`、`session_id`、`query`、`city_id`、`top_k`、`created_at` | 记录一次检索请求，便于审计与评估 |
| `retrieval_hits` | `retrieval_run_id`、`chunk_id`、`rank`、`score` | 保存最终用于回答的证据与排序 |

当前开发环境已使用 SQLite 加应用层余弦相似度完成小规模检索：采用确定性的 `local-hash-v1` 文本向量，支持资料录入、审核状态、自动切片、检索运行与命中留痕。生产环境使用 PostgreSQL 与 `pgvector` 保存向量，并建立 `ivfflat` 或 `hnsw` 索引。向量模型版本必须记录在文档或切片元数据中，重换模型时重新生成向量，不能混用。
...
若没有当前城市、命中分数低于阈值或文档不足，系统必须明确说明“目前没有可靠资料”，并回退到已有的本地城市上下文回答；不得让模型补造来源。

### 25.5 Agent 接口与用户体验

新增只读白名单工具 `search_guide_knowledge(city_id, query, topics, top_k=3)`。它只返回已审核片段、来源标题、链接、更新时间和相关度，不返回任意外部 URL 内容。工具输出作为 LLM 上下文，且同样写入 `agent_tool_calls` 或 `retrieval_runs`。

前端普通聊天消息下方仅在实际命中时显示 1 至 3 条“参考资料”，包含标题、来源名称和外链。不要在正文堆叠长引用，也不要向用户展示向量分数、切片编号或内部提示词。规划页面的“规划依据”继续展示工具和校验结果；RAG 依据只说明推荐理由，不参与预算与时间校验。

### 25.6 质量、安全与答辩评估

- 每次回答最多使用 3 个片段，要求引用城市与用户问题匹配；不同城市的资料不得混检。
- 记录检索耗时、命中率、平均相似度、空结果率、来源点击率和带来源回答占比。
- 准备 20 至 30 条城市知识问答，人工标注期望城市、主题和可接受来源，评估 `Recall@3`、引用正确率、回答事实一致率和端到端耗时。
- 文档内容视为不可信输入。检索片段不能覆盖系统提示词、调用非白名单工具、泄露用户资料或指示模型忽略安全规则。
- 管理员删除或撤销文档后，必须同步删除或失效其切片与向量；用户侧历史回答保留当时的来源标题和链接快照，便于追溯。

### 25.7 实施顺序

1. 已完成：建立文档、切片、检索运行和命中记录的数据模型，以及管理员资料录入和审核页；正式部署仍需 Alembic 迁移。
2. 已完成：SQLite 小规模检索接口；待补充 20 至 30 篇真实审核资料和前端来源卡片。
3. 待用户授权将资料发送至外部 LLM 后，将 `search_guide_knowledge` 接入普通聊天，不接入行程事实工具。
4. 增加检索评估集、失败回退和管理员检索日志。
5. 部署 PostgreSQL 后迁移至 `pgvector`，再按真实数据量评估索引参数。

## 28. 行程社区模块

### 28.1 定位与边界

行程社区用于让用户将已有行程发布到平台，附带景点照片和文字说明，供其他登录用户浏览、点赞、收藏和评论。它不同于现有 `share_links`：分享链接是行程所有者主动发送的、可撤销且不公开作者身份的只读链接；社区帖子是可被平台内容流发现的公开内容。

现有 `itinerary_feedback` 是用户对自己行程的评分和反馈，不能作为社区评论复用；现有 `media_assets` 是平台城市和景点资料的受控媒体，不接受普通用户上传。社区互动和用户上传必须拥有独立的数据模型、权限校验与审核流程。

### 28.2 已实现的首期范围

首期只实现以下能力：

- 登录用户从自己已保存的行程创建、编辑、撤回公开帖子；发布时填写标题、正文并选择景点照片。公开内容保存为行程快照，不公开原行程备注、聊天消息、邮箱或精确出行日期。
- 游客可浏览已发布的行程流和详情；登录用户可点赞、收藏与发表一级评论。
- 每名用户对同一帖子最多保留一条点赞和一条收藏；删除或下架帖子后，其互动和图片不再公开展示。
- 管理员可查看和下架/重新公开帖子，并查看和处理举报；后端接口支持隐藏评论和图片。用户可举报帖子、评论或图片。

首期不实现评论回复、用户关注、私信、个性化推荐算法或按热度自动排序。内容流先采用发布时间倒序和城市筛选，确保规则可解释且实现范围可控。

### 28.3 数据模型与图片存储

| 表 | 核心字段 | 用途 |
| --- | --- | --- |
| `community_posts` | `id`、`author_id`、`itinerary_id`、`itinerary_snapshot`、`title`、`body`、`city_name`、`status`、`created_at` | 保存公开行程帖子、发布状态和不可变行程快照 |
| `community_post_images` | `id`、`post_id`、`storage_path`、`mime_type`、`sort_order`、`status` | 保存用户上传的景点照片及展示状态 |
| `community_post_likes` | `user_id`、`post_id`、`created_at`，唯一约束 `user_id + post_id` | 保证每人只能点赞一次 |
| `community_post_favorites` | `user_id`、`post_id`、`created_at`，唯一约束 `user_id + post_id` | 保存社区帖收藏，不与个人行程收藏混用 |
| `community_comments` | `id`、`post_id`、`author_id`、`body`、`status`、`created_at` | 保存一级评论和审核状态 |
| `content_reports` | `id`、`reporter_id`、`target_type`、`target_id`、`reason`、`status` | 保存用户举报和管理员处理结果 |

当前开发环境通过应用启动建表，图片写入 `backend/media/community/`。上传接口只接受经文件头校验的 JPEG、PNG 或 WebP，每张不超过 8MB、每帖最多 9 张；数据库只保存相对路径和服务器识别的 MIME 类型，不信任前端传入的图片 URL、MIME 类型或文件名。正式环境必须改用 Alembic 迁移和对象存储。

### 28.4 实施状态与剩余工作

1. 已完成：帖子列表、详情、发布、编辑、撤回、所有权校验和公开快照接口。
2. 已完成：本地受限图片上传、格式和大小校验，以及媒体静态代理。
3. 已完成：点赞、收藏、一级评论和数据库唯一约束；暂未实现频率限制。
4. 已完成基础举报、帖子下架和管理员操作审计；管理员前端页面当前提供帖子与举报处理。
5. 已完成后端权限、重复互动、撤回和快照脱敏测试，以及前端 TypeScript/Vite 构建；仍需补浏览器端到端测试。

行程回收站在彻底删除前会把社区帖子视为外部关联；存在帖子时只能保留回收站状态，不能删除原行程。后续必须补充 Alembic 迁移、对象存储、EXIF 清理、恶意文件扫描、评论和图片的后台可视化审核、频率限制与更细的内容审核状态流转。首期仍不包含评论回复、用户关注、私信、个性化推荐或按热度自动排序。
