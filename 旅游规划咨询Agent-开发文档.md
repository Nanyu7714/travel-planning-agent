...
| 验证邮箱 | `/verify-email` | 使用邮件中的 6 位验证码激活邮箱 |
...
注册接口只创建未验证账号，不发送邮件；前端随后进入验证页，由用户单独调用发送验证码接口。注册、发送或重新发送验证码和找回密码接口成功时统一返回 `202 Accepted`，响应文案不说明邮箱是否存在。邮箱验证提交目标邮箱和 6 位验证码，验证码 3 分钟失效、60 秒内不能重发、成功后立即作废。密码重置和新邮箱确认链接中的一次性令牌由前端读取后立即使用 `history.replaceState` 清除地址栏参数，再通过 POST 请求提交；相关页面设置 `Referrer-Policy: no-referrer`，避免令牌随 Referer 泄露。
...
收藏写入和删除均保持幂等。城市和景点详情页使用登录用户的最近浏览记录；用户可以一键清空，单条历史不提供永久保存承诺。邮箱验证码、重发验证、找回密码、重置密码和新邮箱确认已经实现；开发环境返回本地测试验证码或测试链接，生产环境通过统一 SMTP 适配器投递。
...
自然语言调整接口兼容原有文本输入，同时接受受 Pydantic Schema 限制的 `actions`：`set_budget`、`set_days`、`remove_attraction`、`replace_attraction`。服务端逐项验证景点城市归属、时间冲突、重复景点和门票预算后，才创建行程版本。
