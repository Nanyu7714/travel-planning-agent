...
import { Archive, ArchiveRestore, Bot, Check, ChevronLeft, ChevronRight, EllipsisVertical, ListChecks, Pencil, Pin, PinOff, Sparkles, Trash2, X } from 'lucide-vue-next'
...
const emit = defineEmits<{
  ...
  toggleSelectionMode: []
  toggleSelection: [session: ChatSession]
  selectAll: []
  bulkArchive: []
  bulkDelete: []
...
function toggleMenu(sessionId: number, event: MouseEvent) {
  ...
  menuOpensUpward.value = spaceBelow < 190
...
function closeMenu() { openMenuId.value = null; menuOpensUpward.value = false }
  ...
  <aside :class="['session-rail', { collapsed }]">
    <div class="rail-heading">
      ...
      <div v-if="!collapsed" class="rail-actions">
        ...
        <button class="icon-button" :aria-label="selectionMode ? '退出批量选择' : '批量选择会话'" :title="selectionMode ? '退出批量选择' : '批量选择会话'" @click="emit('toggleSelectionMode')"><X v-if="selectionMode" :size="17" /><ListChecks v-else :size="17" /></button>
        <button v-if="!selectionMode" class="icon-button" aria-label="新建会话" title="新建会话" @click="emit('create')"><Sparkles :size="17" /></button>
      </div>
    ...
    <div v-if="!collapsed && selectionMode" class="selection-toolbar">
      <button class="selection-all" @click="emit('selectAll')">{{ selectedSessionIds.length === sessions.length && sessions.length ? '取消全选' : '全选' }}</button>
      <span>{{ selectedSessionIds.length }} 已选</span>
      <div class="selection-commands">
        <button class="icon-button" :disabled="!selectedSessionIds.length" :aria-label="archivedView ? '恢复所选会话' : '归档所选会话'" :title="archivedView ? '恢复所选会话' : '归档所选会话'" @click="emit('bulkArchive')"><ArchiveRestore v-if="archivedView" :size="16" /><Archive v-else :size="16" /></button>
        <button class="icon-button danger-action" :disabled="!selectedSessionIds.length" aria-label="删除所选会话" title="删除所选会话" @click="emit('bulkDelete')"><Trash2 :size="16" /></button>
      </div>
    </div>

    <div class="session-scroll">
      <div v-for="session in sessions" :key="session.id" class="session-entry">
        <button :class="['session-item', { active: activeSessionId === session.id, selected: selectionMode && selectedSessionIds.includes(session.id) }]" :title="session.title" :aria-pressed="selectionMode ? selectedSessionIds.includes(session.id) : undefined" @click="selectionMode ? emit('toggleSelection', session) : emit('select', session)">
          <span v-if="selectionMode && !collapsed" class="session-check"><Check v-if="selectedSessionIds.includes(session.id)" :size="13" /></span>
          <Bot v-else :size="16" />
          <span v-if="!collapsed">{{ session.title }}</span><Pin v-if="session.is_pinned && !collapsed && !selectionMode" class="session-pin" :size="13" />
        </button>
        <button v-if="!collapsed && !selectionMode" class="session-menu-trigger" aria-label="管理会话" title="管理会话" @click.stop="toggleMenu(session.id, $event)"><EllipsisVertical :size="16" /></button>
        <div v-if="!collapsed && !selectionMode && openMenuId === session.id" :class="['session-menu', { 'open-upward': menuOpensUpward }]" role="menu" @click.stop>
...
<style scoped>
.session-rail { position: sticky; top: 80px; height: calc(100vh - 80px); min-height: 0; padding: 24px 12px 12px; display: flex; flex-direction: column; overflow: visible; background: var(--color-canvas); transition: padding .18s ease; }
...
.selection-toolbar { min-height: 34px; display: flex; align-items: center; gap: 6px; margin: 0 4px 9px; padding: 5px 2px 8px; border-bottom: 1px solid var(--border); color: var(--secondary); font-size: 11px; }
.selection-all { min-height: 26px; padding: 3px 5px; border: 0; background: transparent; color: var(--primary); font-size: 12px; }
.selection-all:hover { text-decoration: underline; }
.selection-commands { display: flex; gap: 1px; margin-left: auto; }
.selection-commands .icon-button { width: 28px; height: 28px; }
...
.session-entry .session-item.selected { border-color: var(--primary); background: var(--primary-soft); }
.session-check { width: 16px; height: 16px; flex: none; display: grid; place-items: center; border: 1px solid var(--border-strong); border-radius: 4px; color: var(--color-on-primary); }
.session-item.selected .session-check { border-color: var(--primary); background: var(--primary); }
...
.session-menu.open-upward { top: auto; bottom: 41px; }
